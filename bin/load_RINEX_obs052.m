function [pr1, ph1, pr2, ph2, dop1, dop2, snr1, snr2, ...
    ...
    load_RINEX_obs052(filename, constellations, wait_dlg)
% [pr1, ph1, pr2, ph2, dop1, dop2, snr1, snr2, ...
%           time_zero, time_GPS, time, week, date, pos, interval, antoff, antmod, codeC1, marker] = ...
%           load_RINEX_obs052(filename, constellations, processing_interval, wait_dlg)
...
for f = 1 : nFiles
    ...
    [obsColumns, nObsTypes] = obs_type_find052(obs_type, sysId);
    ...
    while (~feof(fid))
        ...
        if ~isempty(date(k,1,f))
            ...
            %             obs = RINEX_get_obs_old(fid, num_sat, sat, sat_types, obsColumns, nObsTypes, constellations);
            obs =RINEX_get_obs052(fid, num_sat, sat, sat_types, obsColumns, nObsTypes, constellations);
            idx_P1 = obs.P1 ~= 0;
            idx_C1 = obs.C1 ~= 0;
    ...
    disp('Done reading current RINEX');
end
...
% log.newLine();
disp('Syncing observations if needed');
% sum()
%sync observations
...
disp('Trimming short arcs if needed');
if sum( time_ref~= round( time,5))
    time_ref=time;
end
