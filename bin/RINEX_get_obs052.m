function [obs_struct] = RINEX_get_obs052(file_RINEX, nSat, sat, sat_types, obs_col, nObsTypes, constellations)
% RINEX_get_obs(buf, cur_line, nSat, sat, sat_types, obs_col, nObsTypes, constellations, active_sys, first_id_sys)
...
%         linTmp = buf{cur_line};
        ...
        switch (sysId)
            case 'G'
                if (constellations.GPS.enabled)%active_sys(1)
%                     index = first_id_sys(1) + satId - 1;
            ...
            case 'R'
                if (constellations.GLONASS.enabled)%active_sys(2)
%                     index = first_id_sys(2) + satId - 1;
            ...
            case 'E'
                if (constellations.Galileo.enabled)%active_sys(3)
%                     index = first_id_sys(3) + satId - 1;
            ...
            case 'J'
                if (constellations.QZSS.enabled)%active_sys(5)
%                     index = first_id_sys(4) + satId - 1;
            ...
            case 'C'
                if (constellations.BeiDou.enabled)%active_sys(4)
...
%             case 'I'
%                 if (constellations.Galileo.enabled)%active_sys(6)
%                     index = first_id_sys(6) + satId - 1;
%                 else
%                     continue
%                 end
            case 'S'
                if (constellations.SBAS.enabled)%active_sys(6)
